...
const LiveJobScan = () => {
  ...
  const calculateSuspiciousScore = (description) => {
    let score = 0;
  ...
  const fetchJobs = async () => {
    setLoading(true);
    const response = await fetch(`https://serpapi.com/search.json?engine=google_jobs&q=work from home&location=United States&hl=en&api_key=${SERPAPI_KEY}`);
    const data = await response.json();
    const jobResults = data.jobs_results || [];

    const results = await Promise.all(
      jobResults.slice(0, 10).map(async (job) => {
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 p-6 border shadow-lg rounded-xl bg-white">
      <h2 className="text-2xl font-bold mb-4">🔍 Live Job Scam Detection</h2>
